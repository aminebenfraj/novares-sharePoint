  ...
  try {
    ...
    try {
      ...
      await Promise.allSettled(
        managerEmailList.map((emailOptions) => sendManagerCreationEmail(emailOptions)),
      );
...
exports.signSharePoint = async (req, res) => {
  try {
    ...
    sharePoint.updateHistory.push({
      action: "signed",
      performedBy: req.user._id,
      details: `Document signed by ${req.user.username}${signatureNote ? ` with note: ${signatureNote}` : ""}`,
...
exports.approveSharePoint = async (req, res) => {
  try {
    ...
    sharePoint.updateHistory.push({
      ...
      performedBy: req.user._id,
      details: approved
        ? `Document approved by manager ${req.user.username}${approvalNote ? ` with note: ${approvalNote}` : ""}`
    ...
    if (approved) {
      ...
      try {
        await sendDisapprovalEmail({
          ...
          documentId: sharePoint._id.toString(),
          disapprovedBy: req.user.username,
          disapprovalNote: approvalNote || "No reason provided",
